...
const LoginForm = () => {
    ...
    const onSubmitHandler = async(e,helpers) => {
        setLoading(true);
        try {
            const response = await axios.post(BACKEND_URI+"/login", e);
        ...
        <div className="">
            <Formik validationSchema={validationSchema} onSubmit={onSubmitHandler} initialValues={initialValues}>
                <Form action="#">
                    ...
                    <Field type="email" name="email" placeholder="Email" className="w-[150%] outline-none p-2 my-2 bg-gray-200 rounded" />
                    <ErrorMessage component={'p'} name="email" className="text-[#fe1d1d]" />
                    <Field type="password" name="password" placeholder="Password" className="w-[150%] outline-none p-2 my-2 bg-gray-200 rounded" />
                    ...
                    <button disabled={loading} type="submit" id="LoginBtn" className="bg-red-500 text-white px-6 py-2 rounded mt-4"><a href="/dashboard">Sign In</a></button>
                </Form>
            </Formik>
        </div>
    );
